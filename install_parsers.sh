...
# Function to install Ansible Content Parser
...
detect_os_and_install() {
    ...
    echo "Please enter the tool you want to install (ansible, terraform, pulumi, aws-cfn-lint, azure-template-analyzer, foodcritic, salt-lint, puppet, all):"
    read tool_choice
    ...
    case $tool_choice in
        ...
        azure-template-analyzer)
            install_azure_template_analyzer_"$os_type"
            ;;
        foodcritic)
            install_foodcritic_"$os_type"
            ;;
        salt-lint)
            install_salt_lint_"$os_type"
            ;;
        puppet)
...
# Run the function to detect OS and install the selected tools
detect_os_and_install
